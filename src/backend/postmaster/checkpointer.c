...
void CheckpointerMain(void) {
    ...
    checkpointer_context = AllocSetContextCreate(
        TopMemoryContext, "Checkpointer", ALLOCSET_DEFAULT_SIZES);
     ...
     */
    for (;;) {
        bool do_checkpoint = false;  // 是否需要做ckpt
        int flags = 0;
        pg_time_t now;
        int elapsed_secs;
        int cur_timeout;
        ...
        ResetLatch(MyLatch);  // latch是用于进程间通信的
        ...
        if (do_checkpoint) {
            ...
            /*
             * 如果pg正在recovery中，那我们就需要做restartpoint
             ...
             */
            SpinLockAcquire(&CheckpointerShmem->ckpt_lck);
            flags |= CheckpointerShmem->ckpt_flags;  // flag标志需要什么ckpt类型
            ...
            /*
             * 如果是recovery末尾&&flag被置位，则需要做restartpoint
            ...
            if (!do_restartpoint && (flags & CHECKPOINT_CAUSE_XLOG) &&
                ...
                ereport(LOG, (errmsg_plural("checkpoints are occurring too "
                                            "frequently (%d second apart)",
             ...
             */
            if (!do_restartpoint) {
                CreateCheckPoint(flags);  // 核心: 真正执行脏页刷盘等操作
                ckpt_performed = true;
            } else
            ...
            if (ckpt_performed) {
                 ...
                 * 因为没有找到WAL文件，所以restartpoint未成功，则每15s继续尝试一次restartpoint
                 */
        ...
        /*
         * cur_timeout：据下一次ckpt，还需等待多久，通过WaitLatch实现等待
...
static bool IsCheckpointOnSchedule(double progress) {
    ...
    elapsed_xlogs =
        (((double)(recptr - ckpt_start_recptr)) / wal_segment_size) /
        CheckPointSegments;
...
#define MAX_SIGNAL_TRIES 600 /* max wait 60.0 sec */
    ...
    if (flags & CHECKPOINT_WAIT) {
        ...
        if (new_failed != old_failed)
            ereport(ERROR, (errmsg("checkpoint request failed"),
                            errhint("Consult recent messages in the server log "
                                    "for details.")));
    }
}
...
void AbsorbSyncRequests(void) {
    ...
    if (n > 0) {
        requests =
            (CheckpointerRequest*)palloc(n * sizeof(CheckpointerRequest));
